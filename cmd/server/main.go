...
func main() {
	...
	if value, ok := lookupEnv("MONGOSTORE_URI", "mongostore_uri", "MONGO_URI", "mongo_uri"); ok {
		envMongoStoreConfigured = true
		mongoStoreURI = value
	}
	if value, ok := lookupEnv("MONGOSTORE_DATABASE", "mongostore_database", "MONGO_DATABASE", "mongo_database"); ok {
		envMongoStoreConfigured = true
	...
	if useMongoStore {
		...
		mongoStoreInst, err = store.NewMongoStore(ctx, store.MongoStoreConfig{
			...
			Database:   effectiveMongoDatabase,
			Collection: effectiveMongoCollection,
		})
		cancel()
		if err != nil {

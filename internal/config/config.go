...
type Config struct {
	...
	ErrorCooldowns ErrorCooldowns `yaml:"error-cooldowns" json:"error-cooldowns"`
	...
	// AntigravitySignatureBypassStrict controls whether bypass mode uses strict protobuf-tree validation.
...
type RemoteManagement struct {
	...
	// Management panel remote auto-update is disabled globally and this flag is ignored.
	DisableAutoUpdatePanel bool `yaml:"disable-auto-update-panel"`
	// PanelGitHubRepository is retained for configuration compatibility only.
	// Management panel remote fetching is disabled globally and this value is ignored.
	PanelGitHubRepository string `yaml:"panel-github-repository"`
}

// ErrorCooldowns configures cooldown durations for different upstream failure categories.
// All values are in seconds. Nil means "use built-in default". Zero means "no cooldown".
type ErrorCooldowns struct {
	PaymentRequiredSeconds   *int `yaml:"payment-required-seconds,omitempty" json:"payment-required-seconds,omitempty"`
	NotFoundSeconds          *int `yaml:"not-found-seconds,omitempty" json:"not-found-seconds,omitempty"`
	ModelNotSupportedSeconds *int `yaml:"model-not-supported-seconds,omitempty" json:"model-not-supported-seconds,omitempty"`
	TransientErrorSeconds    *int `yaml:"transient-error-seconds,omitempty" json:"transient-error-seconds,omitempty"`
	RateLimitBaseSeconds     *int `yaml:"rate-limit-base-seconds,omitempty" json:"rate-limit-base-seconds,omitempty"`
...
type RoutingConfig struct {
	...
	// Deprecated: use SessionAffinity for universal session routing.
	...
	// Accepts Go duration strings such as "30m" or "1h".
...
func (cfg *Config) SanitizeErrorCooldowns() {
	if cfg == nil {
		return
	}

	fields := []*int{
		cfg.ErrorCooldowns.PaymentRequiredSeconds,
		cfg.ErrorCooldowns.NotFoundSeconds,
		cfg.ErrorCooldowns.ModelNotSupportedSeconds,
		cfg.ErrorCooldowns.TransientErrorSeconds,
		cfg.ErrorCooldowns.RateLimitBaseSeconds,
		cfg.ErrorCooldowns.RateLimitMaxSeconds,
	}

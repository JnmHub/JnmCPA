...
func NewWatcher(configPath, authDir string, reloadCallback func(*config.Config)) (*Watcher, error) {
	...
	if store := sdkAuth.GetTokenStore(); store != nil {
		w.watchAuthDir = sdkAuth.UsesAuthFileMirror(store)
		w.watchAuthDirSet = true
		...
		if !w.watchAuthDir {
			log.Info("auth file mirror disabled for token store; watcher will skip auth-dir scans and file events")
		}
	}
	return w, nil
}

func (w *Watcher) shouldWatchAuthDir() bool {
	if w == nil {
		return true
	}
	if !w.watchAuthDirSet {
		return true
	}
	return w.watchAuthDir
}

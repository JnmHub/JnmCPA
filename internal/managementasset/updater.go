...
// StartAutoUpdater previously launched a background asset updater.
// Remote auto-update is disabled and management assets are served from local disk only.
func StartAutoUpdater(ctx context.Context, configFilePath string) {
	_ = ctx
	_ = configFilePath
	log.Debug("management asset auto-updater disabled: local assets only")
}
...
// EnsureLatestManagementHTML now only checks whether the local management asset exists.
// Remote download and auto-update are intentionally disabled.

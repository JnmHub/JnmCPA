...
func TestAntigravityExecutor_StripsUnsignedThinkingBlocksBeforeValidation(t *testing.T) {
	payload := []byte(`{"messages":[{"role":"assistant","content":[{"type":"thinking","thinking":"bad","signature":""},{"type":"text","text":"ok"}]}]}`)
	from := sdktranslator.FromString("claude")
	...
	if bytes.Contains(out, []byte(`"thinking"`)) {
		t.Fatalf("unsigned thinking block should be stripped, got %s", string(out))
	}
}

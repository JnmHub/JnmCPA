...
func NewUtlsHTTPClient(cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	var fallback http.RoundTripper = &http.Transport{
	...
	if proxyURL != "" {
		if transport, _, errBuild := proxyutil.BuildHTTPTransport(proxyURL); errBuild != nil {
			log.Errorf("utls: failed to configure fallback proxy transport for %q: %v", proxyURL, errBuild)

...
type authFileListResult struct {
	Files      []gin.H
	Total      int
	Page       int
	PageSize   int
...
func (h *Handler) ListAuthFiles(c *gin.Context) {
	...
	if h.authManager == nil {
		h.listAuthFilesFromDisk(c, query)
		return
	}
	if shouldProbeAuthFiles(c) {
		h.probeVisibleAuthFiles(c.Request.Context())
	}
...
func parseAuthFileListQuery(c *gin.Context) (authFileListQuery, error) {
	...
	if pageSizeRaw := strings.TrimSpace(c.Query("page_size")); pageSizeRaw != "" {
		...
		if pageSize > maxAuthFilesPageSize {
			pageSize = maxAuthFilesPageSize
		}
		query.PageSize = pageSize
	}

	query.Provider = strings.ToLower(strings.TrimSpace(firstNonEmptyQueryValue(c, "provider", "type", "filter")))
	query.Search = strings.TrimSpace(c.Query("search"))
	query.StatusCode = strings.TrimSpace(c.Query("status_code"))

	if sortMode := strings.TrimSpace(c.Query("sort")); sortMode != "" {
		switch sortMode {
	...
	if value, ok, err := optionalBoolQuery(c, "problem_only"); err != nil {
		return query, err
	} else if ok {
		query.ProblemOnly = value
	}

	if value, ok, err := optionalBoolQuery(c, "disabled"); err != nil {
		return query, err
	} else if ok {
		query.Disabled = &value
	}

	if value, ok, err := optionalBoolQuery(c, "usable"); err != nil {
		return query, err
	} else if ok {
		query.Usable = &value
	}

	if value, ok, err := optionalBoolQuery(c, "cooling"); err != nil {
		return query, err
	} else if ok {
		query.Cooling = &value
	}
...
func optionalBoolQuery(c *gin.Context, key string) (bool, bool, error) {
	if c == nil {
...
func (h *Handler) collectManagedAuthFileEntries() []gin.H {
...
func buildAuthFileListResponse(entries []gin.H, query authFileListQuery) gin.H {
...
func filterAuthFileEntries(entries []gin.H, query authFileListQuery) authFileListResult {
	baseFiltered := make([]gin.H, 0, len(entries))
	typeCounts := map[string]int{"all": 0}

	searchMatcher := compileAuthFileWildcardMatcher(query.Search)
	for _, entry := range entries {
		if !matchesAuthFileQuery(entry, query, searchMatcher, true) {
			continue
		}
		baseFiltered = append(baseFiltered, entry)
		typeCounts["all"]++
		if fileType := authFileEntryString(entry, "type", "provider"); fileType != "" {
			typeCounts[fileType]++
		}
	}

	filtered := make([]gin.H, 0, len(baseFiltered))
	for _, entry := range baseFiltered {
		if !matchesAuthFileProvider(entry, query.Provider) {
			continue
		}
		filtered = append(filtered, entry)
	}

	sortAuthFileEntries(filtered, query.Sort)
...
func compileAuthFileWildcardMatcher(search string) *regexp.Regexp {
...
func matchesAuthFileQuery(entry gin.H, query authFileListQuery, searchMatcher *regexp.Regexp, ignoreProvider bool) bool {
	if !ignoreProvider && !matchesAuthFileProvider(entry, query.Provider) {
		return false
	}
	if query.ProblemOnly && !authFileEntryHasProblem(entry) {
		return false
	}
	if query.Disabled != nil {
		if value, ok := authFileEntryBool(entry, "disabled"); !ok || value != *query.Disabled {
			return false
		}
	}
	if query.Usable != nil {
		if value, ok := authFileEntryBool(entry, "usable"); !ok || value != *query.Usable {
			return false
		}
	}
	if query.Cooling != nil {
		if value, ok := authFileEntryBool(entry, "cooling"); !ok || value != *query.Cooling {
			return false
		}
	}
	if !matchesAuthFileStatusCode(entry, query.StatusCode) {
		return false
	}
	if !matchesAuthFileSearch(entry, query.Search, searchMatcher) {
		return false
	}
	return true
}

func matchesAuthFileProvider(entry gin.H, provider string) bool {
	provider = strings.ToLower(strings.TrimSpace(provider))
	if provider == "" || provider == "all" {
		return true
	}
	fileType := strings.ToLower(strings.TrimSpace(authFileEntryString(entry, "type", "provider")))
	return fileType == provider
}

func matchesAuthFileSearch(entry gin.H, search string, searchMatcher *regexp.Regexp) bool {
	search = strings.TrimSpace(search)
	if search == "" {
		return true
	}
	...
	for _, candidate := range authFileSearchCandidates(entry) {
		if candidate == "" {
			continue
		}
		if searchMatcher != nil {
			if searchMatcher.MatchString(candidate) {
				return true
			}
			continue
		}
		if strings.Contains(strings.ToLower(candidate), lowerSearch) {
			return true
		}
	}
	return false
...
func authFileSearchCandidates(entry gin.H) []string {
	fields := []string{
		...
		authFileEntryString(entry, "error_code"),
		authFileEntryString(entry, "error_message"),
	}
	if statusCode, ok := authFileEntryInt(entry, "status_code"); ok {
		fields = append(fields, strconv.Itoa(statusCode))
	}
	return fields
}

func matchesAuthFileStatusCode(entry gin.H, filter string) bool {
	filter = strings.TrimSpace(filter)
	if filter == "" {
		return true
	}

	statusCode, ok := authFileEntryInt(entry, "status_code")
	if !ok {
		return false
	}
	...
	for _, token := range strings.Split(filter, ",") {
		...
		if len(token) == 3 && strings.HasSuffix(token, "xx") {
			prefix := token[0]
			if prefix >= '1' && prefix <= '5' && statusCode/100 == int(prefix-'0') {
				return true
			}
			continue
		}
		exact, err := strconv.Atoi(token)
		if err == nil && exact == statusCode {
			return true
		}
	}
	return false
...
func sortAuthFileEntries(entries []gin.H, sortMode string) {
	sort.Slice(entries, func(i, j int) bool {
		...
		switch sortMode {
		case "priority":
			leftPriority := authFileEntryPriority(left)
			rightPriority := authFileEntryPriority(right)
			if leftPriority != rightPriority {
				return leftPriority > rightPriority
			}
		case "az":
			// handled by name fallback below
		default:
			leftProvider := strings.ToLower(authFileEntryString(left, "provider", "type"))
			rightProvider := strings.ToLower(authFileEntryString(right, "provider", "type"))
			if leftProvider != rightProvider {
				return leftProvider < rightProvider
			}
		}

		leftName := strings.ToLower(authFileEntryString(left, "name"))
		rightName := strings.ToLower(authFileEntryString(right, "name"))
		return leftName < rightName
	})
}

func authFileEntryPriority(entry gin.H) int {
	if value, ok := entry["priority"]; ok {
...
func authFileEntryHasProblem(entry gin.H) bool {
	if statusCode, ok := authFileEntryInt(entry, "status_code"); ok {
		if statusCode >= 400 {
			return true
		}
		if statusCode > 0 {
			return strings.TrimSpace(authFileEntryString(entry, "error_code", "error_message")) != ""
		}
	}
	if strings.TrimSpace(authFileEntryString(entry, "error_code", "error_message")) != "" {
		return true
	}

	status := strings.ToLower(strings.TrimSpace(authFileEntryString(entry, "status")))
	switch status {
	case "error", "failed", "invalid":
		return true
	}

	message := strings.ToLower(strings.TrimSpace(authFileEntryString(entry, "status_message")))
	if message == "" {
		return false
	}
	_, healthy := healthyAuthFileMessages[message]
	return !healthy
}

func authFileEntryString(entry gin.H, keys ...string) string {
	for _, key := range keys {
...
func authFileEntryBool(entry gin.H, key string) (bool, bool) {
	value, ok := entry[key]
	if !ok {
		return false, false
	}
	...
	case bool:
		return typed, true
	case string:
		parsed, err := parseFlexibleBool(typed)
		if err == nil {
			return parsed, true
		}
	case int:
...
func authFileEntryInt(entry gin.H, key string) (int, bool) {
	value, ok := entry[key]
	if !ok {
		return 0, false
	}
...
func shouldProbeAuthFiles(c *gin.Context) bool {
	if c == nil {
		return false
	}
	switch strings.ToLower(strings.TrimSpace(c.Query("probe"))) {
...
func (h *Handler) probeVisibleAuthFiles(ctx context.Context) {
	if h == nil || h.authManager == nil {
		return
	}
...
func shouldProbeAuth(auth *coreauth.Auth) bool {
	if auth == nil {
		return false
	}
...
func (h *Handler) probeAuthFile(ctx context.Context, auth *coreauth.Auth) {
	if h == nil || h.authManager == nil || auth == nil {
		return
	}

	provider := authProviderKey(auth)
	model, routeable := probeModelForAuth(auth)
	if provider == "" || model == "" {
		return
	}

	payload, err := buildAuthProbePayload(model)
	if err != nil {
		log.WithError(err).Debugf("failed to build auth probe payload for %s", auth.ID)
		return
	}
	...
	if routeable {
		_, err = h.authManager.Execute(probeCtx, []string{provider}, req, opts)
		if err != nil {
			log.WithError(err).Debugf("auth probe failed for %s (%s)", auth.ID, model)
		}
		return
	}
	...
	_, err = executor.Execute(probeCtx, auth, req, opts)
	...
	if err != nil {
		result.Error = coreauth.NormalizeError(err)
		log.WithError(err).Debugf("auth probe failed for %s (%s)", auth.ID, model)
	}
	h.authManager.MarkResult(probeCtx, result)
}

func authProviderKey(auth *coreauth.Auth) string {
	if auth == nil {
		return ""
	}
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	storageBackend := strings.TrimSpace(authAttribute(auth, "storage_backend"))
	if path == "" && !runtimeOnly && storageBackend == "" {
		return nil
	}
	...
	entry := gin.H{
		...
		"usable":           authUsable(auth, now),
		"cooling":          authCooling(auth, now),
	}
	applyAuthResultFields(entry, auth)
	if storageBackend != "" {
		entry["source"] = storageBackend
	}
...
func authCooling(auth *coreauth.Auth, now time.Time) bool {
...
func authUsable(auth *coreauth.Auth, now time.Time) bool {
	if auth == nil {
		return false
	}
	if auth.Disabled || auth.Status == coreauth.StatusDisabled {
...
func applyAuthResultFields(entry gin.H, auth *coreauth.Auth) {
	if len(entry) == 0 || auth == nil {
		return
	}

	result := auth.LastResult
	if result == nil && auth.LastError != nil {
		result = &coreauth.ResultSnapshot{
			Success:    false,
	...
	if statusCode := result.StatusCode; statusCode > 0 {
		entry["status_code"] = statusCode
		lastResult["status_code"] = statusCode
	}
	...
	if !result.UpdatedAt.IsZero() {
		lastResult["updated_at"] = result.UpdatedAt
	}
	...
	lastError := gin.H{}
	if statusCode := result.StatusCode; statusCode > 0 {
		lastError["http_status"] = statusCode
	}
	if code := strings.TrimSpace(result.Code); code != "" {
		lastError["code"] = code
	}
...
func (h *Handler) DeleteAuthFile(c *gin.Context) {
	...
	if len(names) == 0 {
		query, errQuery := parseAuthFileListQuery(c)
		if errQuery != nil {
		...
		if all := c.Query("all"); all == "true" || all == "1" || all == "*" {
			if authFileListQueryHasFilters(query) {
				names = h.authFileNamesMatchingQuery(query)
			} else {
				deleted := 0
				names = make([]string, 0)
				if store := h.tokenStoreWithBaseDir(); sdkAuth.UsesAuthFileMirror(store) {
					entries, err := os.ReadDir(h.cfg.AuthDir)
					if err != nil {
						c.JSON(500, gin.H{"error": fmt.Sprintf("failed to read auth dir: %v", err)})
						return
		...
		} else if authFileListQueryHasFilters(query) {
			names = h.authFileNamesMatchingQuery(query)
		}
	}

	if len(names) == 0 {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "deleted": 0, "files": []string{}})
		return
	}
	...
	if len(names) == 1 {
		...
		c.JSON(http.StatusOK, gin.H{"status": "ok", "deleted": 1, "files": []string{filepath.Base(names[0])}})
		return
	}
...
func authFileListQueryHasFilters(query authFileListQuery) bool {
	return query.Provider != "" ||
		...
		query.Usable != nil ||
		query.Cooling != nil ||
		strings.TrimSpace(query.StatusCode) != ""
}

func (h *Handler) authFileNamesMatchingQuery(query authFileListQuery) []string {
	query.Page = 1
	query.PageSize = 0
	result := filterAuthFileEntries(h.collectManagedAuthFileEntries(), query)
	names := make([]string, 0, len(result.Files))
	for _, entry := range result.Files {
		if name := authFileEntryString(entry, "name"); name != "" {
			names = append(names, name)
		}
	}
...
func (h *Handler) writeAuthFile(ctx context.Context, name string, data []byte) error {
	...
	if store := h.tokenStoreWithBaseDir(); !sdkAuth.UsesAuthFileMirror(store) {
		_, err = h.saveTokenRecord(ctx, auth)
		return err
	}
...
func (h *Handler) deleteAuthFileByName(ctx context.Context, name string) (string, int, error) {
	...
	if sdkAuth.UsesAuthFileMirror(store) {
		...
		deleteID := targetID
		if deleteID == "" {
			deleteID = h.authIDForPath(targetPath)
		}
		if deleteID == "" {
			deleteID = filepath.Base(name)
		}
		if errDeleteRecord := store.Delete(ctx, deleteID); errDeleteRecord != nil {
			return filepath.Base(name), http.StatusInternalServerError, errDeleteRecord
		}
	}
	skipPersistCtx := coreauth.WithSkipPersist(ctx)
	if targetID != "" {
		h.disableAuth(skipPersistCtx, targetID)
		h.emitRuntimeAuthDelete(ctx, targetID)
	} else {
		h.disableAuth(skipPersistCtx, targetPath)
		h.emitRuntimeAuthDelete(ctx, h.authIDForPath(targetPath))
	}
...
func (h *Handler) upsertAuthRecord(ctx context.Context, auth *coreauth.Auth) error {
	...
	registered, err := h.authManager.Register(ctx, auth)
	if err == nil {
		h.emitRuntimeAuthUpsert(ctx, watcher.AuthUpdateActionAdd, registered)
	}
	return err
}
...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	updated, err := h.authManager.Update(ctx, targetAuth)
	if err != nil {
...
func (h *Handler) emitRuntimeAuthUpsert(ctx context.Context, action watcher.AuthUpdateAction, auth *coreauth.Auth) {
	if h == nil || h.runtimeAuthHook == nil || auth == nil || auth.ID == "" {
		return
	}
	h.runtimeAuthHook(coreauth.WithSkipPersist(ctx), watcher.AuthUpdate{
		Action: action,
		ID:     auth.ID,
		Auth:   auth.Clone(),
	})
...
func (h *Handler) emitRuntimeAuthDelete(ctx context.Context, id string) {
	if h == nil || h.runtimeAuthHook == nil {
		return
	}
	id = strings.TrimSpace(id)
	if id == "" {
...
func (h *Handler) saveTokenRecord(ctx context.Context, record *coreauth.Auth) (string, error) {
	...
	if h.authManager != nil {
		if _, ok := h.authManager.GetByID(record.ID); ok {
	...
	h.emitRuntimeAuthUpsert(ctx, action, record)

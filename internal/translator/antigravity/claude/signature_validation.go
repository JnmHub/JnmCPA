...
// InspectClaudeBypassSignature exposes the decoded signature tree for diagnostics/tests.
func InspectClaudeBypassSignature(rawSignature string) (*claudeSignatureTree, error) {
	sig := strings.TrimSpace(rawSignature)
	if sig == "" {
	...
	if sig == "" {
		return nil, fmt.Errorf("empty signature after stripping prefix")
	}

	switch sig[0] {
	case 'R':
		return inspectDoubleLayerSignature(sig)
	case 'E':
		return inspectSingleLayerSignature(sig)
	default:
		return nil, fmt.Errorf("invalid signature: expected 'E' or 'R' prefix, got %q", string(sig[0]))
	}
}

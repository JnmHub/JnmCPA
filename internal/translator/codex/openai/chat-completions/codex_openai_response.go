...
func ConvertCodexResponseToOpenAI(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if dataType == "response.reasoning_summary_text.delta" {
		...
		imageURL := "data:" + mimeTypeFromCodexOutputFormat(outputFormat) + ";base64," + b64

		if imagesResult := gjson.GetBytes(template, "choices.0.delta.images"); !imagesResult.Exists() || !imagesResult.IsArray() {
		...
		if itemType == "image_generation_call" {
			...
			imageURL := "data:" + mimeTypeFromCodexOutputFormat(itemResult.Get("output_format").String()) + ";base64," + b64
			if imagesResult := gjson.GetBytes(template, "choices.0.delta.images"); !imagesResult.Exists() || !imagesResult.IsArray() {
				template, _ = sjson.SetRawBytes(template, "choices.0.delta.images", []byte(`[]`))
			}
			imageIndex := len(gjson.GetBytes(template, "choices.0.delta.images").Array())
	...
	if outputResult.IsArray() {
		...
		for _, outputItem := range outputArray {
			...
			case "image_generation_call":
				...
				imagePayload, _ = sjson.SetBytes(imagePayload, "image_url.url", "data:"+mimeTypeFromCodexOutputFormat(outputItem.Get("output_format").String())+";base64,"+b64)

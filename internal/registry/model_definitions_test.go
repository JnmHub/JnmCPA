...
func TestCodexStaticModelsIncludeGPT55(t *testing.T) {
	...
	channelModels := GetStaticModelDefinitionsByChannel("codex")
	channelModel := findModelInfo(channelModels, "gpt-5.5")
	if channelModel == nil {
		t.Fatal("expected codex channel static definitions to include gpt-5.5")
	}
	assertGPT55ModelInfo(t, "channel", channelModel)
}

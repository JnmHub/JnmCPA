...
type Service struct {
	...
	// storeSyncCancel cancels direct-store change subscription for non-mirrored stores.
	storeSyncCancel context.CancelFunc
...
func (s *Service) usesMirroredAuthFiles() bool {
	...
	return sdkAuth.UsesAuthFileMirror(tokenStore)
}

func (s *Service) syncDirectStoreAuths(ctx context.Context, action watcher.AuthUpdateAction) {
	if s == nil || s.coreManager == nil || s.usesMirroredAuthFiles() {
		return
	}
	if ctx == nil {
...
func (s *Service) startDirectStoreSync(ctx context.Context) {
	if s == nil || s.usesMirroredAuthFiles() {
		return
	}
	store := sdkAuth.GetTokenStore()
	subscriber, ok := store.(sdkAuth.AuthChangeSubscriber)
	if !ok {
		return
	}
	if ctx == nil {
	...
	if s.storeSyncCancel != nil {
		return
	}
	syncCtx, cancel := context.WithCancel(ctx)
	s.storeSyncCancel = cancel
	go func() {
		if err := subscriber.SubscribeAuthChanges(syncCtx, s.handleDirectStoreAuthChange); err != nil && !errors.Is(err, context.Canceled) {
			log.Errorf("direct auth store sync stopped: %v", err)
		}
	}()
}

func (s *Service) handleDirectStoreAuthChange(change sdkAuth.AuthChange) {
	if s == nil {
		return
	}
	...
	case sdkAuth.AuthChangeActionUpsert:
		...
		if s.coreManager != nil {
			if _, ok := s.coreManager.GetByID(change.Auth.ID); ok {
				action = watcher.AuthUpdateActionModify
			}
		}
		s.emitAuthUpdate(ctx, watcher.AuthUpdate{Action: action, ID: change.Auth.ID, Auth: change.Auth.Clone()})
	}
}
...
func (s *Service) applyCoreAuthRemoval(ctx context.Context, id string) {
	...
	if err := s.coreManager.Delete(coreauth.WithSkipPersist(ctx), id); err != nil {
...
func (s *Service) Run(ctx context.Context) error {
	...
	s.syncDirectStoreAuths(ctx, watcher.AuthUpdateActionAdd)
	s.startDirectStoreSync(ctx)

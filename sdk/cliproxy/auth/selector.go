...
var sessionPattern = regexp.MustCompile(`_session_([a-zA-Z0-9-]+)$`)
...
// ExtractSessionID extracts the primary session identifier for affinity routing.
...
func extractSessionIDs(headers http.Header, payload []byte, metadata map[string]any) (string, string) {
	...
	if headers != nil {
		...
		if sid := strings.TrimSpace(headers.Get("Idempotency-Key")); sid != "" {
			return "idempotency:" + sid, ""
		}
	}
	...
	if len(metadata) > 0 {
		if sid := extractMetadataString(metadata, "user_id"); sid != "" {
			return "meta-user:" + sid, ""
		}
		if sid := extractMetadataString(metadata, "conversation_id"); sid != "" {
			return "meta-conv:" + sid, ""
		}
		if sid := extractMetadataString(metadata, "idempotency_key"); sid != "" {
			return "meta-idempotency:" + sid, ""
		}
	}

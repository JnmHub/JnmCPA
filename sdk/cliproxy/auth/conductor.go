...
const (
	...
	// success but the auth still evaluates as needing refresh.
...
type errorCooldownPolicy struct {
	paymentRequired   time.Duration
	notFound          time.Duration
	modelNotSupported time.Duration
...
func errorCooldownPolicyFromConfig(cfg *internalconfig.Config) errorCooldownPolicy {
	policy := errorCooldownPolicy{
		paymentRequired:   30 * time.Minute,
		notFound:          12 * time.Hour,
		modelNotSupported: 12 * time.Hour,
		transient:         1 * time.Minute,
		rateLimitBase:     quotaBackoffBase,
		rateLimitMax:      quotaBackoffMax,
	}
	if cfg == nil {
		return policy
	}
	policy.paymentRequired = configuredDuration(cfg.ErrorCooldowns.PaymentRequiredSeconds, policy.paymentRequired)
	policy.notFound = configuredDuration(cfg.ErrorCooldowns.NotFoundSeconds, policy.notFound)
	policy.modelNotSupported = configuredDuration(cfg.ErrorCooldowns.ModelNotSupportedSeconds, policy.modelNotSupported)
	policy.transient = configuredDuration(cfg.ErrorCooldowns.TransientErrorSeconds, policy.transient)
	policy.rateLimitBase = configuredDuration(cfg.ErrorCooldowns.RateLimitBaseSeconds, policy.rateLimitBase)
	policy.rateLimitMax = configuredDuration(cfg.ErrorCooldowns.RateLimitMaxSeconds, policy.rateLimitMax)
	return policy
}

func addCooldown(now time.Time, duration time.Duration) time.Time {
	if duration <= 0 {
		return time.Time{}
	}
...
type Result struct {
	...
	Success bool
	// StatusCode carries an explicit upstream HTTP-like status when known.
	StatusCode int
	// Code carries an optional structured result code.
	Code string
	// Message carries an optional structured result message.
	Message string
...
type selectorStopper interface {
	Stop()
}
...
func (m *Manager) SetSelector(selector Selector) {
	...
	if stopper, ok := previous.(selectorStopper); ok && previous != nil && previous != selector {
		stopper.Stop()
	}
}
...
// Delete removes one auth from runtime state and optionally from the backing store.
// When the context carries WithSkipPersist, only in-memory/runtime state is removed.
func (m *Manager) Delete(ctx context.Context, id string) error {
	...
	idKeyPrefix := strings.ToLower(id) + "|"
	m.mu.Lock()
	delete(m.auths, id)
	for key := range m.modelPoolOffsets {
		if strings.HasPrefix(key, idKeyPrefix) {
	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				failed = true
				rerr := normalizeResultError(chunk.Err)
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	cfg, _ := m.runtimeConfig.Load().(*internalconfig.Config)
	policy := errorCooldownPolicyFromConfig(cfg)

	m.mu.Lock()
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		now := time.Now()
		auth.LastResult = snapshotResult(result, now)
		...
		if result.Success {
			...
			if result.Model != "" {
				...
				if isModelSupportResultError(result.Error) {
					next := addCooldown(now, policy.modelNotSupported)
					...
					switch statusCode {
					case 401:
						next := addCooldown(now, policy.paymentRequired)
						state.NextRetryAfter = next
					...
					case 402, 403:
						...
						shouldSuspendModel = true
					case 404:
						next := addCooldown(now, policy.notFound)
						state.NextRetryAfter = next
					...
					case 429:
						...
						if result.RetryAfter != nil {
							...
							cooldown, nextLevel := nextQuotaCooldown(
								backoffLevel,
								quotaCooldownDisabledForAuth(auth),
								policy.rateLimitBase,
								policy.rateLimitMax,
							)
							if cooldown > 0 {
					...
					case 408, 500, 502, 503, 504:
						if quotaCooldownDisabledForAuth(auth) {
							...
							next := addCooldown(now, policy.transient)
							state.NextRetryAfter = next
						}
					default:
						state.NextRetryAfter = time.Time{}
				...
				applyAuthFailureState(auth, result.Error, result.RetryAfter, now, policy)
			}
		}
...
func snapshotResult(result Result, now time.Time) *ResultSnapshot {
	...
	if result.Success {
		...
		snapshot.Code = strings.TrimSpace(result.Code)
		snapshot.Message = strings.TrimSpace(result.Message)
		return snapshot
	}

	normalized := cloneError(result.Error)
	if normalized == nil {
		normalized = &Error{}
	}
	if normalized.HTTPStatus <= 0 && result.StatusCode > 0 {
		normalized.HTTPStatus = result.StatusCode
	}
	if normalized.Code == "" {
		normalized.Code = strings.TrimSpace(result.Code)
	}
	if normalized.Message == "" {
		normalized.Message = strings.TrimSpace(result.Message)
	}
	snapshot.StatusCode = normalized.HTTPStatus
	snapshot.Code = strings.TrimSpace(normalized.Code)
	snapshot.Message = strings.TrimSpace(normalized.Message)
	snapshot.Retryable = normalized.Retryable
	return snapshot
}

func normalizeResultError(err error) *Error {
	if err == nil {
		return nil
	}
	...
	if normalized.Message == "" {
		normalized.Message = strings.TrimSpace(err.Error())
	}
	if normalized.HTTPStatus <= 0 {
		normalized.HTTPStatus = statusCodeFromError(err)
	}

	if code, message := structuredErrorDetails(normalized.Message); code != "" || message != "" {
	...
	if normalized.Code == "" {
		normalized.Code = inferResultErrorCode(err, normalized.HTTPStatus, normalized.Message)
	}
	if normalized.HTTPStatus <= 0 {
		normalized.HTTPStatus = inferSyntheticStatusCode(normalized.Code, normalized.Message)
	}
	if normalized.Message == "" && normalized.HTTPStatus > 0 {
		normalized.Message = strings.TrimSpace(http.StatusText(normalized.HTTPStatus))
	}
	if normalized.Message == "" {
		normalized.Message = strings.TrimSpace(err.Error())
	}
...
// NormalizeError converts arbitrary executor/runtime errors into the structured
// auth error shape used by management and scheduler state updates.
func NormalizeError(err error) *Error {
	return normalizeResultError(err)
}

func structuredErrorDetails(message string) (code string, detail string) {
	...
	targets := []map[string]any{root}
	if nested, ok := root["error"].(map[string]any); ok && len(nested) > 0 {
		targets = append([]map[string]any{nested}, targets...)
	}

	for _, target := range targets {
		if code == "" {
			code = firstStructuredString(target, "code", "type", "error_code", "errorType")
		}
		if detail == "" {
			detail = firstStructuredString(target, "message", "detail", "error_message", "description")
		}
	}
...
func inferResultErrorCode(err error, status int, message string) string {
...
func inferSyntheticStatusCode(code string, message string) int {
	switch strings.TrimSpace(strings.ToLower(code)) {
	case "invalid_request_error", "bad_request", "unsupported_parameter":
	...
	case "bad_gateway", "upstream_closed":
		return http.StatusBadGateway
	case "service_unavailable", "dns_error", "connection_refused", "tls_error", "network_error":
	...
	lower := strings.ToLower(strings.TrimSpace(message))
	switch {
	case strings.Contains(lower, "unsupported parameter"), strings.Contains(lower, "invalid_request_error"):
		return http.StatusBadRequest
	case strings.Contains(lower, "timeout"), strings.Contains(lower, "deadline exceeded"):
		return http.StatusRequestTimeout
	case strings.Contains(lower, "connection refused"), strings.Contains(lower, "no such host"), strings.Contains(lower, "tls:"):
		return http.StatusServiceUnavailable
	}
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, policy errorCooldownPolicy) {
	...
	case 401:
		...
		auth.NextRetryAfter = addCooldown(now, policy.paymentRequired)
	case 402, 403:
		auth.StatusMessage = "payment_required"
		auth.NextRetryAfter = addCooldown(now, policy.paymentRequired)
	case 404:
		auth.StatusMessage = "not_found"
		auth.NextRetryAfter = addCooldown(now, policy.notFound)
	case 429:
	...
	case 408, 500, 502, 503, 504:
		...
		if quotaCooldownDisabledForAuth(auth) {
			...
			auth.NextRetryAfter = addCooldown(now, policy.transient)
		}
	default:
...
func nextQuotaCooldown(prevLevel int, disableCooling bool, baseCooldown, maxCooldown time.Duration) (time.Duration, int) {
	if prevLevel < 0 {
		prevLevel = 0
	}
	if disableCooling || baseCooldown <= 0 || maxCooldown <= 0 {
		return 0, prevLevel
	}
...
func (m *Manager) tryAntigravityCreditsExecute(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, bool) {
	...
	for _, candidate := range candidates {
		...
		publishSelectedAuthMetadata(creditsOpts.Metadata, candidate.auth.ID)

		models := m.executionModelCandidates(candidate.auth, routeModel)
		if len(models) == 0 {
			continue
		}
		pooled := len(models) > 1
		for _, upstreamModel := range models {
			...
			execReq := req
			execReq.Model = upstreamModel
			resp, errExec := candidate.executor.Execute(creditsCtx, candidate.auth, execReq, creditsOpts)
			result := Result{AuthID: candidate.auth.ID, Provider: candidate.provider, Model: resultModel, Success: errExec == nil}
			if errExec != nil {
				if m.shouldRecordAuthFailureForError(errExec) {
					result.Error = normalizeResultError(errExec)
					if ra := retryAfterFromError(errExec); ra != nil {
...
func (m *Manager) tryAntigravityCreditsExecuteStream(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, bool) {
	...
	for _, candidate := range candidates {
		...
		result, errStream := m.executeStreamWithModelPool(creditsCtx, candidate.executor, candidate.auth, candidate.provider, req, creditsOpts, routeModel, models, len(models) > 1)
		if errStream != nil {
			continue
		}
		return result, true
...
func (m *Manager) StopAutoRefresh() {
	...
	if stopper, ok := m.selector.(selectorStopper); ok && stopper != nil {
		stopper.Stop()
	}
}
...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	...
	if err != nil {
		...
		if current := m.auths[id]; current != nil {
			...
			current.LastError = normalizeResultError(err)
			m.auths[id] = current

...
func TestManagerExecute_OpenAICompatAliasPoolStopsOnModelSupportBadRequestWhenRetryDisabled(t *testing.T) {
	alias := "claude-opus-4.66"
	modelSupportErr := &Error{
		HTTPStatus: http.StatusBadRequest,
		Message:    "invalid_request_error: The requested model is not supported.",
	...
	m.SetConfig(&internalconfig.Config{
		...
		RetryModelNotSupported: boolPtr(false),
	})

	_, err := m.Execute(context.Background(), []string{"pool"}, cliproxyexecutor.Request{Model: alias}, cliproxyexecutor.Options{})
	if err == nil {
		t.Fatalf("expected unsupported-model error without fallback")
	}
	got := executor.ExecuteModels()
	if len(got) != 1 || got[0] != "qwen3.5-plus" {
		t.Fatalf("execute calls = %v, want only first unsupported model", got)
	}
}

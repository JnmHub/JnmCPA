...
func (b *Builder) Build() (*Service, error) {
	...
	if coreManager == nil {
		...
		selector := newCoreAuthSelector(b.cfg)
		coreManager = coreauth.NewManager(tokenStore, selector, authdeletestats.DefaultManager())
	}
	...
	if !sdkAuth.UsesAuthFileMirror(tokenStore) {
		service.serverOptions = append(service.serverOptions, api.WithRuntimeAuthHook(func(ctx context.Context, update watcher.AuthUpdate) {
			service.emitAuthUpdate(coreauth.WithSkipPersist(ctx), update)
		}))
	}
	return service, nil
}

...
func (a *imageSSEAccumulator) AddChunk(chunk []byte) [][]byte {
	...
	if imageSSENeedsLineBreak(a.pending, chunk) {
	...
	var frames [][]byte
	for {
		frameLen := imageSSEFrameLen(a.pending)
		if frameLen == 0 {
			break
		}
	...
	if len(a.pending) == 0 || !imageSSECanEmitWithoutDelimiter(a.pending) {
		return frames
	}
...
func (a *imageSSEAccumulator) Flush() [][]byte {
	...
	if imageSSECanEmitWithoutDelimiter(a.pending) {
		frames = append(frames, a.pending)
	}
	a.pending = nil
	return frames
...
func imageSSEFrameLen(chunk []byte) int {
	if len(chunk) == 0 {
		return 0
	}
...
func imageSSEHasField(chunk []byte, prefix []byte) bool {
	s := chunk
	for len(s) > 0 {
		line := s
...
func imageSSENeedsMoreData(chunk []byte) bool {
	trimmed := bytes.TrimSpace(chunk)
	if len(trimmed) == 0 {
		return false
	}
	return imageSSEHasField(trimmed, []byte("event:")) && !imageSSEHasField(trimmed, []byte("data:"))
}

func imageSSECanEmitWithoutDelimiter(chunk []byte) bool {
	trimmed := bytes.TrimSpace(chunk)
	if len(trimmed) == 0 || imageSSENeedsMoreData(trimmed) || !imageSSEHasField(trimmed, []byte("data:")) {
		return false
	}
	s := trimmed
	for len(s) > 0 {
...
func imageSSENeedsLineBreak(pending, chunk []byte) bool {
...
func (h *OpenAIAPIHandler) streamImagesFromResponses(c *gin.Context, responsesReq []byte, responseFormat string, streamPrefix string) {
	...
	for {
		...
		case chunk, ok := <-dataChan:
			...
			setSSEHeaders()
			handlers.WriteUpstreamHeaders(c.Writer.Header(), upstreamHeaders)
			h.forwardImagesStream(cliCtx, c, func(err error) { cliCancel(err) }, dataChan, errChan, chunk, responseFormat, streamPrefix, writeEvent)
			return
		}
	}
}

func (h *OpenAIAPIHandler) forwardImagesStream(ctx context.Context, c *gin.Context, cancel func(error), data <-chan []byte, errs <-chan *interfaces.ErrorMessage, firstChunk []byte, responseFormat string, streamPrefix string, writeEvent func(string, []byte)) {
	acc := &imageSSEAccumulator{}
	...
	emitError := func(errMsg *interfaces.ErrorMessage) {
		...
		writeEvent("error", handlers.BuildErrorResponseBody(status, errText))
	}

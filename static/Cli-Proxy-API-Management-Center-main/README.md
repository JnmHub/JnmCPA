# Jnm API Management Center

A single-file Web UI (React + TypeScript) for operating and troubleshooting the **Jnm API** via its **Management API** (config, credentials, logs, and usage).
...
- This repository is the Web UI only. It talks to the Jnm API **Management API** (`/v0/management`) to read/update config, upload credentials, view logs, and inspect usage.
- It is **not** a proxy and does not forward traffic.

## Quick start

### Option A: Use the Web UI bundled in Jnm API (recommended)

1. Start your Jnm API service.
...
Open `http://localhost:5173`, then connect to your Jnm API backend instance.
...
- For Jnm API bundling, the release workflow renames it to `management.html`.

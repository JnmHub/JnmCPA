...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const authUpload = raw['auth-upload'] ?? raw.authUpload;
  if (isRecord(authUpload)) {
    ...
    config.authUpload = {
      maxJsonSizeMb: normalizeInt(authUpload['max-json-size-mb'] ?? authUpload.maxJsonSizeMb),
      maxArchiveSizeMb: normalizeInt(authUpload['max-archive-size-mb'] ?? authUpload.maxArchiveSizeMb),
      maxArchiveEntries: normalizeInt(authUpload['max-archive-entries'] ?? authUpload.maxArchiveEntries),
      maxExpandedSizeMb: normalizeInt(authUpload['max-expanded-size-mb'] ?? authUpload.maxExpandedSizeMb),
    };
  }
  const authProbeModels = raw['auth-probe-models'] ?? raw.authProbeModels;
  if (isRecord(authProbeModels)) {
    const normalized: Record<string, string> = {};
    Object.entries(authProbeModels).forEach(([provider, model]) => {
    ...
    if (Object.keys(normalized).length > 0) {
      config.authProbeModels = normalized;
    }
  }
  ...
  if (typeof maxRetryCredentials === 'number' && Number.isFinite(maxRetryCredentials)) {
    config.maxRetryCredentials = maxRetryCredentials;
  } else if (typeof maxRetryCredentials === 'string' && maxRetryCredentials.trim() !== '') {
    const parsed = Number(maxRetryCredentials);
    if (Number.isFinite(parsed)) {
      config.maxRetryCredentials = parsed;
    }
  }

  const maxRetryInterval = raw['max-retry-interval'] ?? raw.maxRetryInterval;
  if (typeof maxRetryInterval === 'number' && Number.isFinite(maxRetryInterval)) {
    config.maxRetryInterval = maxRetryInterval;
  } else if (typeof maxRetryInterval === 'string' && maxRetryInterval.trim() !== '') {
    const parsed = Number(maxRetryInterval);
    if (Number.isFinite(parsed)) {
      config.maxRetryInterval = parsed;
    }
  }
  const retryModelNotSupported = normalizeBoolean(
    raw['retry-model-not-supported'] ?? raw.retryModelNotSupported
  );
  ...
  if (isRecord(errorCooldowns)) {
    const normalizeIntField = (value: unknown): number | undefined => {
      if (typeof value === 'number' && Number.isFinite(value)) return value;
      if (typeof value === 'string' && value.trim() !== '') {
        const parsed = Number(value);
        if (Number.isFinite(parsed)) return parsed;
    ...
    config.errorCooldowns = {
      paymentRequiredSeconds: normalizeIntField(
        errorCooldowns['payment-required-seconds'] ?? errorCooldowns.paymentRequiredSeconds
      ),
      notFoundSeconds: normalizeIntField(
        errorCooldowns['not-found-seconds'] ?? errorCooldowns.notFoundSeconds
      ),
      modelNotSupportedSeconds: normalizeIntField(
        errorCooldowns['model-not-supported-seconds'] ?? errorCooldowns.modelNotSupportedSeconds
      ),
      transientErrorSeconds: normalizeIntField(
        errorCooldowns['transient-error-seconds'] ?? errorCooldowns.transientErrorSeconds
      ),
      rateLimitBaseSeconds: normalizeIntField(
        errorCooldowns['rate-limit-base-seconds'] ?? errorCooldowns.rateLimitBaseSeconds
      ),
      rateLimitMaxSeconds: normalizeIntField(
        errorCooldowns['rate-limit-max-seconds'] ?? errorCooldowns.rateLimitMaxSeconds
      ),
    };
  }

...
import type { AuthFilesListQuery, AuthFilesResponse } from '@/types/authFile';
import type { OAuthModelAliasEntry } from '@/types';

type StatusError = { status?: number };
type AuthFileStatusResponse = { status: string; disabled: boolean };
type AuthFileFieldsPatchRequest = {
  name: string;
  prefix?: string;
  proxy_url?: string;
  priority?: number;
...
const AUTH_FILE_UPLOAD_TIMEOUT_MS = 15 * 60 * 1000;

export type AuthFileCountResponse = {
  total?: number;
...
export type AuthProbeSingleResponse = {
  status?: string;
  name?: string;
  auth_id?: string;
  provider?: string;
  ...
  probe_model?: string;
  probe_model_source?: 'registry' | 'model_state' | 'default' | string | null;
};

export type AuthResetRetryResponse = {
  status?: string;
  reset?: number;
  total?: number;
};
...
const normalizeTypeCounts = (value: unknown): Record<string, number> => {
  ...
  Object.entries(value as Record<string, unknown>).forEach(([key, entryValue]) => {
    const parsed = readInteger(entryValue);
    counts[key] = parsed && parsed > 0 ? parsed : 0;
  });
...
const buildListParams = (query?: AuthFilesListQuery): Record<string, string | number | boolean> => {
  if (!query) return {};

  const params: Record<string, string | number | boolean> = {};
  if (typeof query.page === 'number' && Number.isFinite(query.page)) {
    params.page = Math.max(1, Math.trunc(query.page));
  }
  if (typeof query.pageSize === 'number' && Number.isFinite(query.pageSize)) {
    params.page_size = Math.max(0, Math.trunc(query.pageSize));
  }
  ...
  if (query.problemOnly === true) {
    params.problem_only = true;
  }
  if (typeof query.disabled === 'boolean') {
    params.disabled = query.disabled;
  }
  if (typeof query.usable === 'boolean') {
    params.usable = query.usable;
  }
  if (typeof query.cooling === 'boolean') {
    params.cooling = query.cooling;
  }
  if (typeof query.statusCode === 'string' && query.statusCode.trim()) {
    params.status_code = query.statusCode.trim();
  }
...
const dedupeAuthFilesResponse = (payload: RawAuthFilesResponse): AuthFilesResponse => {
  ...
  return {
    ...
    page: readInteger(payload?.page) ?? 1,
    pageSize: readInteger(payload?.page_size) ?? 0,
    totalPages: readInteger(payload?.total_pages) ?? 1,
    typeCounts: normalizeTypeCounts(payload?.type_counts),
  };
};
...
export const authFilesApi = {
  list: async (query?: AuthFilesListQuery) =>
    dedupeAuthFilesResponse(
      await apiClient.get<RawAuthFilesResponse>('/auth-files', { params: buildListParams(query) })
    ),

  count: (query?: AuthFilesListQuery) =>
    apiClient.get<AuthFileCountResponse>('/auth-files/count', { params: buildListParams(query) }),

  probeOne: (name: string) =>
    apiClient.post<AuthProbeSingleResponse>('/auth-files/probe', {
      name: String(name ?? '').trim(),
    }),

  resetAllRetryTimes: (query?: AuthFilesListQuery) =>
    apiClient.post<AuthResetRetryResponse>('/auth-files/retry/reset-all', {
      query: query ?? {},
    }),

  exportByQuery: (query?: AuthFilesListQuery) =>
    apiClient.getRaw('/auth-files/export', {
      params: buildListParams(query),
      responseType: 'blob',
    }),

  startProbeBatch: (query?: AuthFilesListQuery) =>
    apiClient.post<{ status?: string; job?: AuthProbeBatchJob }>('/auth-files/probe-batch', {
      query: query ?? {},
    }),

  getProbeBatch: (id: string) =>
    apiClient.get<{ job?: AuthProbeBatchJob }>(`/auth-files/probe-batch/${encodeURIComponent(id)}`),

  setStatus: (name: string, disabled: boolean) =>
    apiClient.patch<AuthFileStatusResponse>('/auth-files/status', { name, disabled }),

  patchFields: (payload: AuthFileFieldsPatchRequest) =>
    apiClient.patch<{ status?: string }>('/auth-files/fields', payload),
  ...
  deleteFile: async (name: string): Promise<AuthFileBatchDeleteResult> => {
    const trimmed = String(name ?? '').trim();
    if (!trimmed) {
      return { status: 'ok', deleted: 0, files: [], failed: [] };
    }
    const payload = await apiClient.delete<AuthFileBatchDeleteResponse>(
      `/auth-files?name=${encodeURIComponent(trimmed)}`
    );
    return normalizeBatchDeleteResponse(payload, [trimmed]);
  },

  deleteAll: () => apiClient.delete('/auth-files', { params: { all: true } }),

  deleteByQuery: async (query: AuthFilesListQuery): Promise<AuthFileBatchDeleteResult> => {
    const params = { ...buildListParams(query), all: true };

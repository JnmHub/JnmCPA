...
const parseStatusMessagePayload = (file: AuthFileItem): Record<string, unknown> | null => {
...
export const getAuthFileErrorMessage = (file: AuthFileItem): string => {
  const payload = parseStatusMessagePayload(file);
  const payloadError = asRecord(payload?.error);
  const lastResult = asRecord(file['last_result'] ?? file.lastResult);
  const lastError = asRecord(file['last_error'] ?? file.lastError);
...
export const getAuthFileErrorCode = (file: AuthFileItem): string => {
  const payload = parseStatusMessagePayload(file);
  const payloadError = asRecord(payload?.error);
  const lastResult = asRecord(file['last_result'] ?? file.lastResult);
  const lastError = asRecord(file['last_error'] ?? file.lastError);
...
export const getAuthFileStatusCode = (file: AuthFileItem): number | null => {
  const payload = parseStatusMessagePayload(file);
  const payloadError = asRecord(payload?.error);
  const lastResult = asRecord(file['last_result'] ?? file.lastResult);
  const lastError = asRecord(file['last_error'] ?? file.lastError);
...
export const getAuthFileNextRetryAfter = (file: AuthFileItem): string | null => {
  const raw = file['next_retry_after'] ?? file.nextRetryAfter;
...
export const getAuthFileCooling = (file: AuthFileItem): boolean => {
  const explicit = readNullableBoolean(file['cooling'] ?? file.cooling);
  if (explicit !== null) return explicit;

  const nextRetryAfter = getAuthFileNextRetryAfter(file);
  if (!nextRetryAfter) return false;

  const retryAt = Date.parse(nextRetryAfter);
  return Number.isFinite(retryAt) && retryAt > Date.now();
};

export const getAuthFileUsable = (file: AuthFileItem): boolean => {
  const explicit = readNullableBoolean(file['usable'] ?? file.usable);
  if (explicit !== null) return explicit;
  return file.disabled !== true && !getAuthFileCooling(file);
};

export const formatAuthFileNextRetryAfter = (file: AuthFileItem): string => {
  const value = getAuthFileNextRetryAfter(file);

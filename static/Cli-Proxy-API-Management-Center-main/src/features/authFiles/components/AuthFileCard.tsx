...
import {
  formatAuthFileNextRetryAfter,
  QUOTA_PROVIDER_TYPES,
  formatModified,
  getAuthFileCooling,
  getAuthFileErrorCode,
...
export function AuthFileCard(props: AuthFileCardProps) {
  ...
  const statusMessage = getAuthFileErrorMessage(file);
  const statusCode = getAuthFileStatusCode(file);
  const errorCode = getAuthFileErrorCode(file);
  const usable = getAuthFileUsable(file);
  const cooling = getAuthFileCooling(file);
  const nextRetryAfter = formatAuthFileNextRetryAfter(file);
  const hasObservedStatus =
    statusCode !== null || Boolean(errorCode) || Boolean(statusMessage) || Boolean(rawStatusMessage);
  const hasStatusWarning =
    (statusCode !== null && statusCode >= 400) ||
    Boolean(errorCode) ||
    (Boolean(statusMessage) && !HEALTHY_STATUS_MESSAGES.has(statusMessage.toLowerCase()));
  const boolText = (value: boolean) => (value ? t('common.yes') : t('common.no'));
  const emptyText = t('auth_files.status_meta_empty', { defaultValue: t('common.not_set') });
    ...
    >
      <div className={styles.fileCardLayout}>
        <div className={styles.fileCardMain}>
          ...
          <div className={`${styles.statusMeta} ${compact ? styles.statusMetaCompact : ''}`}>
            <div
              className={`${styles.statusMetaPill} ${usable ? styles.statusMetaPillSuccess : styles.statusMetaPillDanger}`}
            >
              <span className={styles.statusMetaLabel}>{t('auth_files.status_meta_usable_label')}</span>
              <span className={`${styles.statusMetaValue} ${styles.statusMetaValueMono}`}>
                {boolText(usable)}
              </span>
            </div>
            <div
              className={`${styles.statusMetaPill} ${cooling ? styles.statusMetaPillDanger : styles.statusMetaPillSuccess}`}
            >
              <span className={styles.statusMetaLabel}>
                {t('auth_files.status_meta_cooling_label')}
              </span>
              <span className={`${styles.statusMetaValue} ${styles.statusMetaValueMono}`}>
                {boolText(cooling)}
              </span>
            ...
            <div
              className={`${styles.statusMetaPill} ${
                statusCode === null
                  ? styles.statusMetaPillNeutral
                  : statusCode >= 400
                    ? styles.statusMetaPillDanger
                    : styles.statusMetaPillSuccess
              }`}
            >
              <span className={styles.statusMetaLabel}>
                {t('auth_files.status_meta_status_code_label')}
              </span>
              <span className={`${styles.statusMetaValue} ${styles.statusMetaValueMono}`}>
                {statusCode === null ? emptyText : String(statusCode)}
              </span>
            </div>
            <div
              className={`${styles.statusMetaPill} ${
                errorCode ? styles.statusMetaPillDanger : styles.statusMetaPillNeutral
              }`}
            >
              <span className={styles.statusMetaLabel}>
                {t('auth_files.status_meta_error_code_label')}
              </span>
              <span className={`${styles.statusMetaValue} ${styles.statusMetaValueMono}`}>
                {errorCode || emptyText}
              </span>
            </div>
            <div className={`${styles.statusMetaPill} ${styles.statusMetaPillNeutral}`}>
              <span className={styles.statusMetaLabel}>
                {t('auth_files.status_meta_next_retry_after_label')}
              </span>
              <span className={`${styles.statusMetaValue} ${styles.statusMetaValueMono}`}>
                {nextRetryAfter === 'null' ? emptyText : nextRetryAfter}
              </span>
            </div>
          </div>

          {statusMessage && hasStatusWarning && (
            <div className={styles.healthStatusMessage} title={rawStatusMessage || statusMessage}>

...
export function VisualConfigEditor({
  ...
  const authUploadMaxJsonSizeError = getValidationMessage(
    t,
    validationErrors?.['authUpload.maxJsonSizeMb']
  );
  const authUploadMaxArchiveSizeError = getValidationMessage(
    t,
    validationErrors?.['authUpload.maxArchiveSizeMb']
  );
  const authUploadMaxArchiveEntriesError = getValidationMessage(
    t,
    validationErrors?.['authUpload.maxArchiveEntries']
  );
  const authUploadMaxExpandedSizeError = getValidationMessage(
    t,
    validationErrors?.['authUpload.maxExpandedSizeMb']
  );
  const probeBatchConcurrencyError = getValidationMessage(
    t,
    validationErrors?.probeBatchConcurrency
  );
  const requestRetryError = getValidationMessage(t, validationErrors?.requestRetry);
  const maxRetryCredentialsError = getValidationMessage(t, validationErrors?.maxRetryCredentials);
  const maxRetryIntervalError = getValidationMessage(t, validationErrors?.maxRetryInterval);
  const paymentRequiredCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.paymentRequiredSeconds']
  );
  const notFoundCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.notFoundSeconds']
  );
  const modelNotSupportedCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.modelNotSupportedSeconds']
  );
  const transientCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.transientErrorSeconds']
  );
  const rateLimitBaseCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.rateLimitBaseSeconds']
  );
  const rateLimitMaxCooldownError = getValidationMessage(
    t,
    validationErrors?.['cooldowns.rateLimitMaxSeconds']
  );
          ...
          >
            <SectionStack>
              ...
              <SectionGrid>
                ...
                />
                <Input
                  label={t('config_management.visual.sections.remote.operator_secret_key')}
                  type="password"
                  placeholder={t(
                    'config_management.visual.sections.remote.operator_secret_key_placeholder'
                  )}
                  value={values.rmOperatorSecretKey}
                  onChange={(e) => onChange({ rmOperatorSecretKey: e.target.value })}
                  disabled={disabled}
                />
                <Input
                  label={t('config_management.visual.sections.remote.panel_title')}
                  placeholder={t(
                    'config_management.visual.sections.remote.panel_title_placeholder'
                  )}
                  value={values.rmPanelTitle}
                  onChange={(e) => onChange({ rmPanelTitle: e.target.value })}
                  disabled={disabled}
                />
                <Input
          ...
          >
            <SectionStack>
              <SectionGrid>
                ...
                />
                <Input
                  label={t('config_management.visual.sections.auth.upload_max_json_size')}
                  type="number"
                  placeholder="10"
                  value={values.authUploadMaxJsonSizeMb}
                  onChange={(e) => onChange({ authUploadMaxJsonSizeMb: e.target.value })}
                  disabled={disabled}
                  hint={t('config_management.visual.sections.auth.upload_max_json_size_hint')}
                  error={authUploadMaxJsonSizeError}
                />
                <Input
                  label={t('config_management.visual.sections.auth.upload_max_archive_size')}
                  type="number"
                  placeholder="100"
                  value={values.authUploadMaxArchiveSizeMb}
                  onChange={(e) => onChange({ authUploadMaxArchiveSizeMb: e.target.value })}
                  disabled={disabled}
                  hint={t('config_management.visual.sections.auth.upload_max_archive_size_hint')}
                  error={authUploadMaxArchiveSizeError}
                />
                <Input
                  label={t('config_management.visual.sections.auth.upload_max_archive_entries')}
                  type="number"
                  placeholder="10000"
                  value={values.authUploadMaxArchiveEntries}
                  onChange={(e) => onChange({ authUploadMaxArchiveEntries: e.target.value })}
                  disabled={disabled}
                  hint={t('config_management.visual.sections.auth.upload_max_archive_entries_hint')}
                  error={authUploadMaxArchiveEntriesError}
                />
                <Input
                  label={t('config_management.visual.sections.auth.upload_max_expanded_size')}
                  type="number"
                  placeholder="512"
                  value={values.authUploadMaxExpandedSizeMb}
                  onChange={(e) => onChange({ authUploadMaxExpandedSizeMb: e.target.value })}
                  disabled={disabled}
                  hint={t('config_management.visual.sections.auth.upload_max_expanded_size_hint')}
                  error={authUploadMaxExpandedSizeError}
                />
              </SectionGrid>
              <div className={styles.subsection}>
                <ApiKeysCardEditor
          ...
          >
            <SectionStack>
              <SectionGrid>
                ...
                />
                <Input
                  label={t('config_management.visual.sections.network.probe_batch_concurrency')}
                  type="number"
                  placeholder="1"
                  value={values.probeBatchConcurrency}
                  onChange={(e) => onChange({ probeBatchConcurrency: e.target.value })}
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.probe_batch_concurrency_hint')}
                  error={probeBatchConcurrencyError}
                />
                <div className="form-group">
                  <label>{t('config_management.visual.sections.network.probe_model_defaults')}</label>
                  <textarea
                    className="input"
                    placeholder={t(
                      'config_management.visual.sections.network.probe_model_defaults_placeholder'
                    )}
                    value={values.probeModelDefaults}
                    onChange={(e) => onChange({ probeModelDefaults: e.target.value })}
                    disabled={disabled}
                    rows={6}
                  />
                  <div className="hint">
                    {t('config_management.visual.sections.network.probe_model_defaults_hint')}
                  </div>
                </div>
                <Input
                ...
                />
                <Input
                  label={t('config_management.visual.sections.network.cooldown_payment_required')}
                  type="number"
                  placeholder="1800"
                  value={values.cooldowns.paymentRequiredSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        paymentRequiredSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={paymentRequiredCooldownError}
                />
                <Input
                  label={t('config_management.visual.sections.network.cooldown_not_found')}
                  type="number"
                  placeholder="43200"
                  value={values.cooldowns.notFoundSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        notFoundSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={notFoundCooldownError}
                />
                <Input
                  label={t(
                    'config_management.visual.sections.network.cooldown_model_not_supported'
                  )}
                  type="number"
                  placeholder="43200"
                  value={values.cooldowns.modelNotSupportedSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        modelNotSupportedSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={modelNotSupportedCooldownError}
                />
                <Input
                  label={t('config_management.visual.sections.network.cooldown_transient')}
                  type="number"
                  placeholder="60"
                  value={values.cooldowns.transientErrorSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        transientErrorSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={transientCooldownError}
                />
                <Input
                  label={t('config_management.visual.sections.network.cooldown_rate_limit_base')}
                  type="number"
                  placeholder="1"
                  value={values.cooldowns.rateLimitBaseSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        rateLimitBaseSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={rateLimitBaseCooldownError}
                />
                <Input
                  label={t('config_management.visual.sections.network.cooldown_rate_limit_max')}
                  type="number"
                  placeholder="1800"
                  value={values.cooldowns.rateLimitMaxSeconds}
                  onChange={(e) =>
                    onChange({
                      cooldowns: {
                        ...values.cooldowns,
                        rateLimitMaxSeconds: e.target.value,
                      },
                    })
                  }
                  disabled={disabled}
                  hint={t('config_management.visual.sections.network.cooldown_hint')}
                  error={rateLimitMaxCooldownError}
                />
                <div className={styles.toggleGroup}>
                  <ToggleRow
                    title={t('config_management.visual.sections.network.auto_delete_401')}
                    description={t(
                      'config_management.visual.sections.network.auto_delete_401_desc'
                    )}
                    checked={values.autoDelete401}
                    disabled={disabled}
                    onChange={(autoDelete401) => onChange({ autoDelete401 })}
                  />
                  <ToggleRow
                    title={t('config_management.visual.sections.network.auto_delete_429')}
                    description={t(
                      'config_management.visual.sections.network.auto_delete_429_desc'
                    )}
                    checked={values.autoDelete429}
                    disabled={disabled}
                    onChange={(autoDelete429) => onChange({ autoDelete429 })}
                  />
                  <ToggleRow
                    title={t(
                      'config_management.visual.sections.network.retry_model_not_supported'
                    )}
                    description={t(
                      'config_management.visual.sections.network.retry_model_not_supported_desc'
                    )}
                    checked={values.retryModelNotSupported}
                    disabled={disabled}

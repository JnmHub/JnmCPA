...
用于管理与故障排查 **Jnm API** 的单文件 Web UI（React + TypeScript），通过 **Management API** 完成配置、凭据、日志与统计等管理操作。
...
- 本仓库只包含 Web 管理界面本身，通过 Jnm API 的 **Management API**（`/v0/management`）读取/修改配置、上传凭据、查看日志与使用统计。
- 它 **不是** 代理本体，不参与流量转发。

## 快速开始

### 方式 A：使用 Jnm API 自带的 Web UI（推荐）
...
打开 `http://localhost:5173`，然后连接到你的 Jnm API 后端实例。
...
- 在 Jnm API 的发布流程里会重命名为 `management.html`。
